...
### Changed
- **`bclibc`/`bclibc_flutter` bumped to `^1.0.0-rc.1`**, tracking bclibc core's
  bump to `v2.0.0-rc.1` (the embedded RK45 methods' thread-local
  tolerance/stats API is replaced by stateful integrator classes — see
  bclibc's CHANGELOG). No functional change here: the app only ever selects
  `BcIntegrationMethod.tsitouras` at its default tolerances
  (`lib/core/services/ballistics_service.dart`), never touching the
  tolerance/stats API this replaces.
- **Ballistic engine packages renamed — `dart_bclibc` → [`bclibc`](https://pub.dev/packages/bclibc), `dart_bclibc_flutter` → [`bclibc_flutter`](https://pub.dev/packages/bclibc_flutter), both bumped to `^0.2.3`.** All `package:dart_bclibc(_flutter)/…` imports across `lib/`, `test/` and `packages/ebc_db/` were switched to the new names; `bclibc` is declared as a direct dependency since the app imports it directly in many places. Also fixes a broken import in `ballistics_service_impl_web.dart` (nonexistent `dart_bclibc_flutter.dart` → `bclibc_flutter/bclibc.dart`). Older entries below that mention `dart_bclibc*` refer to the packages under their former names.
- **Material decoupling — migrated from `package:flutter/material.dart`/`cupertino.dart` to the standalone [`material_ui`](https://pub.dev/packages/material_ui)/[`cupertino_ui`](https://pub.dev/packages/cupertino_ui) packages** (Flutter 3.47+). Forced by `go_router ^18.0.1` and `data_table_2 ^3.0.0`, which both moved to `material_ui`: its `MaterialApp`/`Theme`/`DataCell`/`MaterialLocalizations` are distinct types from the framework's, so mixing the two broke the build (`DataCell` type mismatch in `trajectory_table.dart`) and — silently — made `go_router` fail to detect the app's `MaterialApp` and fall back to `NoTransitionPage` (no page transitions, plain error screen). Imports switched app-wide via `dart fix --apply --code=migrate_design_widgets` (113 files across `lib/` and `test/`). Both `MaterialApp`s (`main.dart`, `migration_gate_screen.dart`) now use `material_ui`'s `GlobalMaterialLocalizations.delegates` instead of gen-l10n's `AppLocalizations.localizationsDelegates` (still wired to the legacy `flutter_localizations` ones). App root is wrapped in `MaterialUiCompatibilityBridge` so dependencies still on `flutter/material` (`flutter_markdown_plus`, `window_manager`, `sticky_headers`) keep resolving the app's theme and localizations — temporary, to be dropped once they migrate.
- **`file_picker` bumped `^12.0.0-beta.7` → `^13.0.0`** — single-file imports (`.a7p`/`.ebcp`) use `pickFile()` instead of `pickFiles()` + `.files.single`; the redundant `File(savePath).writeAsBytes(bytes)` after `saveFile()` (which now writes the bytes itself on desktop and returns a `Uri`) was removed from `A7pService`, `EbcpService` and the orphaned-legacy-data export.
- **Dependencies bumped** — `go_router` `^17.4.0` → `^18.0.1`, `data_table_2` `^2.7.2` → `^3.0.0`, `flutter_riverpod` `^3.4.2` → `^3.4.3`, `dart_bclibc_flutter` `^0.2.0` → `^0.2.1`.
- **`analysis_options.yaml`** — `build/` and the platform directories (`android/`, `ios/`, `web/`, `windows/`, `macos/`, `linux/`) are now excluded from analysis.
