...
> **Doc #3 — Status: SUPERSEDED.**  
> All steps here were done, and ObjectBox was the sole storage layer for a
> time — but ObjectBox itself has since been replaced by two embedded
> protobuf files (`packages/ebc_db`, `.ebcp`), and `packages/ebalistyka_db`
> has been deleted. See `8.PROTOBUF_STORAGE_MIGRATION.md` (Doc #8) for the
> current storage architecture. Kept purely as a historical record of why/
> how ObjectBox was adopted.
...
- `List<CoefRow> get coefRows` (from Float64Lists)
...
- `AppSettings` → `GeneralSettings` (with extension)
- `Conditions` → `ShootingConditions` (with extension)
