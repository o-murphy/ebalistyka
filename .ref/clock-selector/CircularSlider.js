...
// }
//
// export default class CircularSlider.tsx.tsx extends PureComponent {
//     static propTypes = {
...
const CircularSlider = ({
    ...
    const [circleCenter, setCircleCenter] = useState({ x: false, y: false });
    ...
    const sleepPanResponder = PanResponder.create({
        ...
        onPanResponderMove: (evt, { moveX, moveY }) => handlePanResponderMove(moveX, moveY),
    });
    ...
    const handlePanResponderMove = (moveX, moveY) => {
        const currentAngleStop = (startAngle + angleLength) % (2 * Math.PI);
        let newAngle = Math.atan2(moveY - circleCenter.y, moveX - circleCenter.x) + Math.PI / 2;
    ...
    //         <Svg height={containerWidth} width={containerWidth}>
    //             <Defs>
    //                 {range(segments).map((i) => {
    ...
    //                 })}
    //
    //                 <G x={`${stop.toX}`} y={`${stop.toY}`} fill={gradientColorTo} transform={{ translate: `${stop.toX}, ${stop.toY}` }} onPressIn={() => setAngleLength(angleLength + Math.PI / 2)} {...wakePanResponder.panHandlers}>
    //                     <Circle r={(strokeWidth - 1) / 2} fill={bgCircleColor} stroke={gradientColorTo} strokeWidth="1" />
    //                     {stopIcon}
    //                 </G>
    //
    //                 <G x={`${start.fromX}`} y={`${start.fromY}`} fill={gradientColorFrom} transform={{ translate: `${start.fromX}, ${start.fromY}` }} onPressIn={() => setStartAngle(startAngle - Math.PI / 2)} {...sleepPanResponder.panHandlers}>
            ...
            <Svg height={containerWidth} width={containerWidth}>
                ...
                >
                    ...
                    <G
                        transform={`translate(${start.fromX}, ${start.fromY})`}
                        fill={gradientColorFrom}
                        onPressIn={() => setStartAngle(startAngle - Math.PI / 2)}
